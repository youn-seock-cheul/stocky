...
class MarketAnalyzer:
    ...
    def extract_portfolio_from_image(self, image_path):
        ...
        try:
            ...
            prompt = """
            당신은 한국 증권사 잔고 스크린샷 전문 분석가입니다. 이미지에서 주식 잔고 정보를 정확히 추출하여 JSON 형식으로 응답하세요.

            [추출 지침]
            1. **종목명(name)**: '종목명' 또는 '유형' 항목 아래에 있는 이름을 추출하세요. (예: 삼성전자, TIGER 미국우주테크)
            2. **티커(ticker)**: 종목명을 기반으로 정확한 티커를 찾으세요. 
               - 한국 주식/ETF는 6자리 숫자 뒤에 .KS(코스피) 또는 .KQ(코스닥)를 반드시 붙이세요. 
               - 예: 삼성전자 -> 005930.KS, TIGER 미국우주테크 -> 479880.KS, ACE AI반도체TOP3+ -> 469150.KS
            3. **평균단가(avg_price)**: '매입가' 또는 '평균단가' 항목의 숫자를 추출하세요. 쉼표(,)와 통화 기호는 제거하세요.
            4. **투자원금(deposit)**: 해당 종목의 총 '매입금액' 또는 (매입가 * 잔고수량)을 계산한 결과를 숫자로 추출하세요.
            
            [주의 사항]
            - 한국 증권사(CYBOS, 키움 영웅문 등) 특유의 한 종목당 데이터가 두 줄로 표시되는 레이아웃을 주의 깊게 분석하세요.
            - 숫자에 포함된 쉼표(,), '원', '%' 등의 문자는 모두 제거하고 순수 숫자만 반환하세요.
            - 결과는 반드시 아래 예시와 같은 JSON 배열 형식이어야 합니다.
            
            응답 형식 예시:
            [
                {"name": "삼성전자", "ticker": "005930.KS", "avg_price": 75000, "deposit": 1500000},
                {"name": "TIGER 미국우주테크", "ticker": "479880.KS", "avg_price": 14435, "deposit": 51374165}
            ]
            """
